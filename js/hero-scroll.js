...
(function(){
  ...
  ready(function(){
    ...
    function mountMedia(scene,index){const holder=scene.querySelector('[data-hero-media]');if(!holder||holder.dataset.loaded==='1'||!data.scenes[index])return;const img=document.createElement('img');img.className='hero-layer hero-layer-main';img.alt='';img.decoding='async';img.loading=index<=1?'eager':'lazy';img.fetchPriority=index===0?'high':'auto';img.src=data.scenes[index].image||FALLBACK_IMAGE;img.style.objectPosition=data.scenes[index].position||'center center';img.onerror=()=>{if(!img.src.endsWith(FALLBACK_IMAGE))img.src=FALLBACK_IMAGE;};holder.appendChild(img);holder.dataset.loaded='1';}
    function mountClosingMedia(){const holder=root.querySelector('[data-closing-media]');if(!holder||holder.dataset.loaded==='1')return;const img=document.createElement('img');img.className='hero-closing-image';img.alt='';img.decoding='async';img.loading='lazy';img.src=data.closing.image||data.scenes[data.scenes.length-1].image||FALLBACK_IMAGE;img.style.objectPosition=data.closing.position||'center center';holder.appendChild(img);holder.dataset.loaded='1';}
    scenes.slice(0,data.scenes.length).forEach((s,i)=>mountMedia(s,i));mountClosingMedia();
    if(reduced){
      viewport.style.position='relative';viewport.style.top='auto';viewport.style.height='auto';viewport.style.overflow='visible';stage.style.height='auto';
      root.querySelector('.hero-index')?.remove();
      scenes.forEach(s=>{s.style.position='relative';s.style.inset='auto';s.style.width='100%';s.style.height='auto';s.style.minHeight='100svh';s.style.visibility='visible';s.style.opacity='1';s.style.transform='none';s.style.pointerEvents='auto';s.style.zIndex='1';s.querySelectorAll('[data-reveal]').forEach(e=>{e.style.opacity='1';e.style.transform='none';});});
      if(closing){closing.style.position='relative';closing.style.inset='auto';closing.style.width='100%';closing.style.height='auto';closing.style.minHeight='100svh';closing.style.marginTop='0';closing.style.padding='110px clamp(32px,9vw,140px)';closing.style.display='flex';closing.style.alignItems='center';closing.style.visibility='visible';closing.style.opacity='1';closing.style.pointerEvents='auto';closing.style.transform='none';closing.style.zIndex='1';}
      return;
    }
    const desktop=()=>window.innerWidth>760;let raf=0,active=-1;const smoothstep=t=>t*t*(3-2*t);
    function setScene(i,opacity,local,visible,revealProgress,z){const scene=scenes[i];scene.style.opacity=String(opacity);scene.style.visibility=visible?'visible':'hidden';scene.style.zIndex=String(z);scene.style.pointerEvents=opacity>.5?'auto':'none';scene.style.transform=visible?`scale(${1.01+Math.abs(local)*.02})`:'scale(1.02)';scene.querySelectorAll('[data-reveal]').forEach((el,n)=>{if(!visible){el.style.opacity='0';el.style.transform='translateY(20px)';return;}const p=Math.max(0,Math.min(1,(revealProgress-.02-n*.025)/.16));el.style.opacity=String(p);el.style.transform=`translateY(${(1-p)*20}px)`;});const img=scene.querySelector('.hero-layer-main');if(img&&visible){const isAI=scene.classList.contains('hero-type-ai');const sceneScale=isAI?0.86:1;img.style.transform=`translate3d(${local*-1.2}%,${local*.5}%,0) scale(${sceneScale*(1.01+Math.abs(local)*.02)})`;}}
