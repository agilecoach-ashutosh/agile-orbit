...
const cards=(arr)=>{
  ...
  const grid=(arr.length===2||arr.length===4)?'2':'3';
  return `<div class="tp-grid-${grid}">${arr.map(c=>`<div class="tp-card">${c.subtitle?`<span class="tp-badge">${esc(c.subtitle)}</span>`:''}<h3>${esc(c.title)}</h3><p>${esc(c.text)}</p></div>`).join('')}</div>`;
};
...
const status=String(m.type).includes('AGILE ORBIT')?'AGILE ORBIT ORIGINAL':'RESEARCHED METHOD';
root.innerHTML=`
<section class="page-hero"><div class="container"><div class="breadcrumbs"><a href="../../">Home</a><span>/</span><a href="../agile-coaching.html">Agile Coaching</a><span>/</span><span>${esc(topicTitle)}</span><span>/</span><span>${esc(m.name)}</span></div><span class="eyebrow">${esc(m.type)} · ${esc(m.theme)} COACHING</span><h1>${esc(m.name)}</h1><p class="hero-lead">${esc(m.summary)}</p><div class="hero-meta"><span>${esc(m.theme)}</span><span>${esc(topicTitle).toUpperCase()}</span><span>${status}</span></div><div class="hero-actions"><a class="btn btn-secondary" href="../agile-coaching.html?topic=${encodeURIComponent(topicId)}#${topicAnchor}">← Back to ${esc(topicTitle)}</a><a class="btn btn-primary" href="#understand">Understand the method ↓</a></div></div></section>
<section class="section"><div class="container technique-shell"><article class="technique-content">
<div class="tp-section" id="understand"><div class="tp-intro"><strong>Core idea:</strong> ${esc(m.helps)}</div>${flow}${formula}<h2 style="margin-top:26px">Understand the method first</h2>${cards(m.components)}${m.components&&m.components.length?'':'<p class="muted">This method is best understood as a whole rather than as separate components.</p>'}<div class="tp-source-note"><strong>Lineage:</strong> ${esc(m.provenance)}</div></div>
