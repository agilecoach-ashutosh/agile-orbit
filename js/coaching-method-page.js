...
root.innerHTML=`
<section class="page-hero"><div class="container"><div class="breadcrumbs"><a href="../../">Home</a><span>/</span><a href="../agile-coaching.html">Agile Coaching</a><span>/</span><span>${esc(m.topicTitle)}</span><span>/</span><span>${esc(m.name)}</span></div><span class="eyebrow">${esc(m.type)} · ${esc(m.theme)} COACHING</span><h1>${esc(m.name)}</h1><p class="hero-lead">${esc(m.summary)}</p><div class="hero-meta"><span>${esc(m.theme)}</span><span>${esc(m.topicTitle).toUpperCase()}</span><span>RESEARCHED METHOD</span></div><div class="hero-actions"><a class="btn btn-secondary" href="../agile-coaching.html?topic=${encodeURIComponent(m.topicId)}#${topicAnchor}">← Back to ${esc(m.topicTitle)}</a><a class="btn btn-primary" href="#understand">Understand the method ↓</a></div></div></section>
