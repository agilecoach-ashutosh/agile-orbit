...
async function fetchPart(name){const base=new URL('.',document.currentScript.src);const url=new URL(name+'?v=bank-v11-20260905',base).href;const r=await fetch(url,{cache:'no-store'});if(!r.ok)throw new Error(url+' returned HTTP '+r.status);const b=await r.arrayBuffer();if(!b.byteLength)throw new Error(url+' returned an empty file');return b;}
