...
async function fetchPart(name){const local=new URL(name+'?v=practice-v11-20260905',location.href).href;try{const r=await fetch(local,{cache:'no-store'});if(!r.ok)throw new Error(local+' returned HTTP '+r.status);const b=await r.arrayBuffer();if(!b.byteLength)throw new Error(local+' returned an empty file');return b}catch(localErr){const remote='https://raw.githubusercontent.com/agilecoach-ashutosh/agile-orbit/main/js/'+name+'?v=practice-v11-20260905';const r=await fetch(remote,{cache:'no-store',mode:'cors'});if(!r.ok)throw new Error('Practice asset unavailable: '+name+' ('+r.status+'); local error: '+localErr.message);const b=await r.arrayBuffer();if(!b.byteLength)throw new Error(remote+' returned an empty file');return b;}}
