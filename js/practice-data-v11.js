/* Agile Orbit Practice — verified 452-question master bank loader. */
(function(){
'use strict';
...
function base64ToBytes(text){const clean=String(text).replace(/\s+/g,'');const binary=atob(clean);const out=new Uint8Array(binary.length);for(let i=0;i<binary.length;i++)out[i]=binary.charCodeAt(i);return out.buffer;}
...
async function fetchPart(name){const url=new URL('../js/'+name+'?v=practice-v13-20260905',location.href).href;const r=await fetch(url,{cache:'no-store',credentials:'same-origin'});if(!r.ok)throw new Error(url+' returned HTTP '+r.status);return base64ToBytes(await r.text());}
