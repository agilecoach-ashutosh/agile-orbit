(()=>{
  ...
  const baseCaps={analysis:4,development:6,test:2};
  ...
  function itemFrom(bp){return {...bp,stage:'ready',remaining:bp.effort.analysis,age:0,startDay:null,doneDay:null,blocked:false,blockLabel:'',blockUntil:0,expedite:false};}

  function init(){
    items=blueprints.map(itemFrom);
    const place=(id,stage,remaining,age,startDay)=>{const x=items.find(i=>i.id===id);x.stage=stage;x.remaining=remaining;x.age=age;x.startDay=startDay;};
  ...
  function currentCaps(){const c={...baseCaps};if(teamMode==='swarm-test'){c.development=4;c.test=4;}if(devPenaltyNextDay>0)c.development=Math.max(2,c.development-1);return c;}
  function avgCycle(){const ds=doneItems().filter(i=>i.doneDay!=null&&i.startDay!=null);return ds.length?ds.reduce((s,i)=>s+(i.doneDay-i.startDay+1),0)/ds.length:0;}
  ...
  function cardMarkup(item){
    const cls=['kfl-card'];if(item.blocked)cls.push('blocked');if(item.expedite)cls.push('expedite');if(item.stage==='done')cls.push('done');
  ...
  function render(){
    ...
    $('capAnalysis').textContent=`${caps.analysis} effort/day`;$('capDevelopment').textContent=`${caps.development} effort/day`;$('capTest').textContent=`${caps.test} effort/day`;
    $('day').textContent=`${day}/${TOTAL_DAYS}`;$('wip').textContent=currentWip();$('done').textContent=doneItems().length;$('cycle').textContent=avgCycle()?avgCycle().toFixed(1)+'d':'—';$('oldest').textContent=oldestWip()?oldestWip()+'d':'—';$('value').textContent=valueDelivered();
    $('wipPolicy').textContent=Number.isFinite(wipLimit)?`WIP limit ${wipLimit}`:'No WIP limit';$('wipPolicy').className='kfl-pill '+(Number.isFinite(wipLimit)?'good':'warn');
    $('teamPolicy').textContent=teamMode==='swarm-test'?'Team swarming on Test':'Specialists stay in lane';$('teamPolicy').className='kfl-pill '+(teamMode==='swarm-test'?'good':'');
    $('blockPolicy').textContent=blockerChoice==='swarm'?'Blocker actively swarmed':blockerChoice==='wait'?'Blocker left unresolved':'No blocker decision yet';$('blockPolicy').className='kfl-pill '+(blockerChoice==='wait'?'warn':blockerChoice==='swarm'?'good':'');
    renderChart();renderLog();
  }

  function setStatus(html){$('status').innerHTML=html;}
  function log(text){decisions.push({day,text});renderLog();}
  function renderLog(){const box=$('logList');if(!box)return;box.innerHTML=decisions.length?decisions.slice().reverse().map(x=>`<div class="kfl-log-item"><b>Day ${x.day}</b> · ${x.text}</div>`).join(''):'<div class="kfl-log-item">Simulation decisions will appear here.</div>';}
  ...
  function workStage(stage,capacity){
    let remainingCap=capacity;const completed=[];
    for(const item of sortForWork(inStage(stage))){if(remainingCap<=0)break;if(item.blocked)continue;const used=Math.min(remainingCap,item.remaining);item.remaining-=used;remainingCap-=used;if(item.remaining<=0)completed.push(item);}
    return completed;
  }
  function advance(completed,fromStage){const next={analysis:'development',development:'test',test:'done'}[fromStage];for(const item of completed){if(next==='done'){item.stage='done';item.doneDay=day;item.remaining=0;}else{item.stage=next;item.remaining=item.effort[next];}}}
  ...
  function pullReady(){
    ...
    for(const item of candidates.slice(0,maxPull)){if(Number.isFinite(wipLimit)&&currentWip()>=wipLimit)break;item.stage='analysis';item.remaining=item.effort.analysis;item.age=0;item.startDay=day;}
    extraPull=0;
  }

  async function simulateDay(){
    if(day>=TOTAL_DAYS)return;day++;ageWip();render();
    setStatus(`<strong>Day ${day} running…</strong><br><span class="muted">Downstream work finishes first. Capacity is then used upstream and Ready work is pulled only if policy allows.</span>`);
    await sleep(500);
    const caps=currentCaps();
    ...
    pullReady();render();recordHistory();if(devPenaltyNextDay>0)devPenaltyNextDay--;await sleep(650);
  }
  ...
  function pauseFor(type){
    ...
    const panel=$('decisionPanel');panel.classList.remove('kfl-hidden');const title=$('decisionTitle'),copy=$('decisionCopy'),choices=$('decisionChoices');
    if(type==='wip'){
      ...
      copy.innerHTML=`Development has ${d} items and Test has <strong>${t} items</strong>. Test is the slower stage at only ${currentCaps().test} effort/day. What should the team do next?`;
      choices.innerHTML=choice('Start more work','Keep Development busy by allowing more items into the system.','bottleneck','start')+choice('Swarm on Test','Shift some Development capacity to help the Test constraint.','bottleneck','swarm');
      ...
      choices.innerHTML=choice('Swarm to unblock','Spend some team capacity now to remove the blocker and restore flow.','blocker','swarm')+choice('Leave it and work elsewhere','The blocked item remains active and continues aging while other work proceeds.','blocker','wait');
    }else if(type==='expedite'){
      title.textContent='Decision 4 · Urgent security patch arrives';
      copy.innerHTML='A production security issue has appeared. The patch is small but urgent. Expedite can shorten its delivery time, but it consumes priority capacity ahead of standard work.';
      choices.innerHTML=choice('Expedite security patch','Admit SEC-1 immediately and give it priority through every stage.','expedite','yes')+choice('Keep normal order','Treat SEC-1 like standard work and protect the existing sequence.','expedite','no');
    }
    panel.scrollIntoView({behavior:'smooth',block:'center'});
  }

  function choose(type,value){
    if(type!==decision)return;$('decisionPanel').classList.add('kfl-hidden');
    if(type==='wip'){
      wipLimit=+value;log(`Set a system WIP limit of ${wipLimit}. New work must now wait when the limit is full.`);
      setStatus(`<strong>WIP limit ${wipLimit} is active.</strong><br><span class="muted">Existing WIP is not discarded. The system simply stops starting new items until space becomes available.</span>`);
    }else if(type==='bottleneck'){
      if(value==='swarm'){
        teamMode='swarm-test';log('Developers began helping Test. Development capacity reduced from 6 to 4; Test increased from 2 to 4.');
        ...
        setStatus('<strong>More work was allowed into the system.</strong><br><span class="muted">Watch whether customer delivery increases—or whether the Test queue simply grows.</span>');
      }
    }else if(type==='blocker'){
      const x=items.find(i=>i.id===blockTarget);blockerChoice=value;
      if(value==='swarm'){
        if(x){x.blocked=false;x.blockLabel='';}devPenaltyNextDay=1;log(`The team swarmed to unblock ${blockTarget}. A little Development capacity will be spent next day to remove waiting.`);
        ...
        if(x)x.blockUntil=TOTAL_DAYS+1;log(`${blockTarget} was left blocked while the team worked elsewhere.`);
        setStatus(`<strong>${blockTarget} remains blocked.</strong><br><span class="muted">It will keep aging and occupying WIP for the rest of this simulation unless someone actively removes the blocker.</span>`);
      }
    }else if(type==='expedite'){
      expediteChoice=value;
      const sec={id:'SEC-1',title:'Production security patch',value:10,effort:{analysis:1,development:2,test:2},stage:'ready',remaining:1,age:0,startDay:null,doneDay:null,blocked:false,blockLabel:'',blockUntil:0,expedite:value==='yes'};items.push(sec);
      if(value==='yes'){
        if(Number.isFinite(wipLimit)&&currentWip()>=wipLimit)wipLimit+=1;
        sec.stage='analysis';sec.remaining=sec.effort.analysis;sec.startDay=day;
        log('SEC-1 was expedited and admitted immediately. It will receive priority through Analysis, Development and Test.');
        setStatus('<strong>SEC-1 is now Expedite.</strong><br><span class="muted">It jumps ahead of standard work. Watch what happens to other aging items while it consumes capacity.</span>');
      }else{
    ...
    decision=null;render();setTimeout(()=>continueRun(),650);
  }
  ...
  async function continueRun(){
    if(running||decision||day>=TOTAL_DAYS)return;running=true;$('startSim').disabled=true;
    while(day<TOTAL_DAYS){await simulateDay();if(shouldPause())return;}
    running=false;finish();
  }

  function finish(){
    $('startSim').disabled=true;
    const end={wip:currentWip(),done:doneItems().length,cycle:avgCycle(),oldest:oldestWip(),value:valueDelivered()};
    $('resultDone').textContent=end.done;$('resultWip').textContent=end.wip;$('resultCycle').textContent=end.cycle?end.cycle.toFixed(1)+'d':'—';$('resultValue').textContent=end.value;
    ...
    $('realizations').innerHTML=`
      <div class="kfl-realization"><strong>1 · WIP changes waiting</strong>${Number.isFinite(wipLimit)?`Your final policy allowed ${wipLimit} active items. Work that could not enter remained Ready instead of becoming aging WIP.`:'No explicit WIP limit was used.'}</div>
      ...
      <div class="kfl-realization"><strong>3 · Blockers consume time</strong>${blockerChoice==='swarm'?`${blockTarget} was actively unblocked rather than left to age.`:`${blockTarget||'The blocked item'} remained stuck and consumed WIP without delivering value.`}</div>
      <div class="kfl-realization"><strong>4 · Expedite has a trade-off</strong>${secText} Expedite can be appropriate for true urgency, but priority capacity is not free.</div>`;
  ...
  function renderChart(){
    const svg=$('cfd');if(!svg)return;const data=history.slice(-11);if(!data.length)return;
    ...
    const x=i=>pad.l+(data.length===1?0:i*(W-pad.l-pad.r)/(data.length-1));const y=v=>H-pad.b-v*(H-pad.t-pad.b)/maxTotal;
    const order=['done','test','development','analysis','ready'];const fills={done:'#3e9f70',test:'#c99845',development:'#4f83bd',analysis:'#6e6ab3',ready:'#536170'};
    let cumulative=data.map(()=>0),paths='';
    for(const key of order){const lower=cumulative.slice();cumulative=cumulative.map((v,i)=>v+data[i][key]);const topPts=data.map((d,i)=>`${x(i)},${y(cumulative[i])}`).join(' ');const botPts=data.map((d,i)=>`${x(data.length-1-i)},${y(lower[data.length-1-i])}`).join(' ');paths+=`<polygon points="${topPts} ${botPts}" fill="${fills[key]}" opacity=".72"></polygon>`;}
  ...
  $('decisionChoices')?.addEventListener('click',e=>{const b=e.target.closest('[data-decision]');if(!b)return;choose(b.dataset.decision,b.dataset.value);});
